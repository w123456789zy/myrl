...
from mylab.rl.alg.base_alg import BaseAlgorithm, OffPolicyBase
...
# ---------------------------------------------------------------------------
# Replay Buffer (vectorized, n_step=1 simplified)
# ---------------------------------------------------------------------------

class FlashSACReplayBuffer:
    """Simple circular replay buffer for vectorized environments.

    Stores transitions from ``num_envs`` parallel envs in a flat circular buffer.
    """
...
class FlashSAC(OffPolicyBase, BaseAlgorithm):
    """FlashSAC off-policy maximum-entropy actor-critic algorithm.

    Adapted from third_part/FlashSAC to the mylab framework.
    """

    #: Auto-registered name (consumed by :data:`ALGORITHM_REGISTRY`).
    algorithm_name: str = "flashsac"
    ...
    def __init__(
        ...
        replay_buffer: FlashSACReplayBuffer,
    ...
    def act(self, obs: torch.Tensor, image: torch.Tensor | None = None) -> torch.Tensor:
        self._prev_obs = obs
        with torch.no_grad():
            actions, _ = self.actor(obs, training=self.is_train_mode)
    ...
    def compute_returns(self, obs: torch.Tensor, image: torch.Tensor | None = None) -> None:
        """No-op for off-policy SAC (returns computed inside replay buffer)."""
        pass
    ...
    def _update_actor(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        actions_all, act_info = self.actor(actor_obs_all, training=True)
        log_probs_all = act_info["log_prob"]

        actions = torch.chunk(actions_all, 2, dim=0)[0]
        log_probs = torch.chunk(log_probs_all, 2, dim=0)[0]

        # Disable critic gradients
        for p in self.critic.parameters():
            p.requires_grad = False
        qs, _ = self.critic(batch["observation"], actions, training=False)
    ...
    def _update_critic(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        with torch.no_grad():
            next_actions, info = self.actor(batch["next_observation"], training=False)
    ...
    # ------------------------------------------------------------------
    # Static helpers (moved from FlashSAC update.py)
    # ------------------------------------------------------------------
    ...
    def _select_min_q_log_probs(
        ...
        """Select log-probs from the min-Q critic."""
    ...
    def construct_algorithm(
        ...
        print(f"FlashSAC Actor  : blocks={actor_num_blocks}, hidden={actor_hidden_dim}")
        print(f"FlashSAC Critic : blocks={critic_num_blocks}, hidden={critic_hidden_dim}, bins={num_bins}")
        print(f"FlashSAC Buffer : capacity={buffer_capacity}, batch={buffer_batch_size}")
