...
"""Play go2-walk with real-time keyboard command input.

Loads a trained checkpoint and lets you control the robot's walking direction
via console input. Default direction is +x.

Usage::

    python test/play_go2_walk.py --env go2-walk --checkpoint logs/model_1000.pt
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Play go2-walk with real-time command input.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--env", type=str, default="go2-walk",
                        help="Environment name (e.g. go2-walk, go2-walk-stairs).")
    ...
    # Detect MuJoCo environments by name
    is_mujoco = args.env.endswith("-mujoco") or args.env in ("go2-rough-mujoco",)

    if is_mujoco:
        from envs.mujoco import get_mujoco_env
        print(f"Creating MuJoCo environment '{args.env}' with {args.num_envs} envs (render ON) ...")
        env = get_mujoco_env(args.env, args.num_envs, show_viewer=True)
    else:
        ...
        print(f"Creating Genesis environment '{args.env}' with {args.num_envs} envs (render ON) ...")
    ...
    # MuJoCo envs reuse the same algorithm config as their Genesis counterpart
    config_env_name = args.env.replace("-mujoco", "")
    train_cfg = get_alg_config(config_env_name, args.alg)
